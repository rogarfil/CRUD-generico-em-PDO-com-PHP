...
class Crud{
    ...
    */
    private function buildInsert($arrayDice){
        ...
        $sql .= "INSERT INTO ".$this->table." (".substr($fields,0, strlen($fields)-1).") VALUES(".substr($values,0, strlen($values)-1).")";
    ...
    */
    ...
    private function buildUpdate($arrayDice, $arrayCondition){
        ...
        // Retira vírgula do final da string
        $valCondition = (substr($valCondition, -2) == 'AND ') ? trim(substr($valCondition,  (strlen($valCondition) - 4))) : $valCondition ;
    ...
    */
    private function buildDelete($arrayCondition){
        ...
        $valFields = (substr($valFields, -4) == 'AND ') ? trim(substr($valFields,  (strlen($valFields) - 4))) : $valFields ;
    ...
    * @param $sql = Instrução SQL inteira contendo, nome das tables envolvidas, JOINS, WHERE, ORDER BY, GROUP BY e LIMIT
